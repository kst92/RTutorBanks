...
ps.name = "BanksEnd"; sol.file = paste0(ps.name,"_sol.Rmd")
...
#< info "plot"
With the function `plot()` you can create several kind of plots, like boxplots or scatterplots. The `abline()` function draws a straight line into a plot, for example a regression line like we did. 
...
where $\alpha_{ct}$ assimilates the county and time effects.
...
Correlation shows the linear relationship between two variables. Changing one variable involves a particular change in the other one. The correlation coefficient between $Y$ and $X$ is defined as:
...
(c.t. Field, Mile, Field (2012), Chapter 6 and Panik (2012), Chapter 12.B)
#>
...
#+ facet_wrap(~Year)
#>
ggplot(data=dat1, aes(x=capitalstock, fill=Year))
...
#< info "felm() function"
`felm()`is a function of the package `lfe` to estimate linear models. The goal here is that we can separate the effects from the normal equation. Therefore, we can use it for huge data sets. It's defined as: `felm(y ~ x1 + x2 | f1 + f2 , data = dataname)`. `y ~ x1+x2` is the regression input, `f1+f2` are the multiple fixed effects.
#>
