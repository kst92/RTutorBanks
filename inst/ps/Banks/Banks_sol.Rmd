...
The package `ggplot2` is a helpful tool to create graphics. With `+` we can simply add the various functions, we want to use in our plot.
`ggplot(data=dat,..)` creates a ggplot object, which mandatory needs a data argument. You can modify there the aesthetics as well, e.g. `aes(x=Year, y=County)`. To plot a special graphic, for example histogram or point-diagram, you have to specify it afterwards. Here are some examples [Tabelle aus Buch Data Analisys,Maindonald, Braun, S.491]
<style>
.mytable {
  border-collapse: collapse;
  margin-bottom: 15px;
...
Moreover it is possible to do some further settings for ggplot objects:
...
.bigger-font-size {
  ...
  <table class="mytable">
  <tr>
  <th class="right-border">Title</th>
  <th>+ opts(title="mytitle)"</th>
  </tr>
  <tr>
  <th class="right-border">Axis label</th>
  <th>+ xlab("myxlab")</th>
  </tr>
  <tr>
  <th class="right-border">Facets Wrap</th>
  <th>+ facet_wrap(~Year)</th>
  </tr>
  </table>
...
#< info "facet_wrap()"
Using `facet_wrap(~var)` out of the package `ggplot2` makes it possible to plot facets next to each other. `var` stands for the variable you want to split up your data.
Follow the link to learn more about it:

https://www.rdocumentation.org/packages/ggplot2/versions/2.2.1/topics/facet_wrap
#>
...
#< info "endogenous effect"
Endogeneity shows the relation between the explanatory (independent) variable and the error term. An endogenous variable collerates with the error term, meaning that the covariance of them is not equal to zero. It is important that no endogeneity is there, otherwise we will receive biased estimates.
#>
...
#< info "linear model lm()"
The function `lm()` is used for representing linear models in its simplest form. It takes two main arguments `formula` and `data`, e.g. `lm(y~x, data=dataname)`.
Read more here: 
...
#>
summary(lm(ln_tval_pc~ capitalstock50pc + factor(year), data =d1))
```
...
#< info "linear model with fixed effects"
...
#< award "Linear Models"
Now you know how to estimate linear models with fixed effects. Keep going on, you are on the right way!
#>
...
```
#< award "Master - Linear Models"
Awesome! You created a regression on your own and can call yourself a Master of Linear Models.
#>
...
#< info "Homoscedasticity & BP-Test"

*Homoscedasticity*
Homoscedasticity means in general `same variance`. It displays that the error term in a linear regression is the same across all values of the independent variables. The opposition is called *Heteroscedasticity*. In that case standard errors would be biased, what would lead to incorrect conclusions about the significance of the regression coefficients. We can test residuals if they are homoscedasticity or heteroscedasticity by using the *Breusch-Pagan -test*.

*BP-Test*

With the Breusch-Pagan-test you can test whether a linear regression model is heteroskedasticity. Therefore, it tests if the variance of the error term is dependent of the independent variables. If the test statisitc has a p-value below an acceptable treshhold e.g. p<0.5, then the null hypothesis of homoskedasticity is rejected and heteroskedasticity assumed.
...
Correlation shows if two variables have a linear relationship. It displays if two variable change together in the same or opposite way.

With the function `cor(x,y)` we can calculate the correlation coefficient.
