...
libs = c("foreign", "ggmap", "dplyr", "ggplot2", "weights", "lmtest", "lfe", "stargazer", "regtools", "yaml", "usmap", "maps", "plm") # character vector of all packages you load in the problem set
...
- yes*
- no
success: Correct! Since the capital amount changed from 0 to 30 (compare the value of capitalstock and capitalstock1902), we can assume that the banks were constrained by the 50.000$ requirement since reducing it to 25.000$ lead to more entering of national banks.
...
To analyse this, we need the data from 1870 to 1900. To reduce the size of our sample we will juts use counties with a capitalstock less than 200.000 $. Since the values in our data frame are given in 1.000 Dollar, we must filter out `capitalstock <= 200`. For this we make use of the function `filter` from the package `dplyr`.
...
#>
ggplot(data=dat1, aes(x=capitalstock, fill=Year))+ geom_histogram(breaks=seq(0, 200, by =10)) + 
  labs(x="County nationalbank capital stock ", y= "Number of counties") + facet_wrap(~Year)
```
#< award "Histogram"
Congratulations! You can plot now nice histogram and know how to use facets. 
#>
...
#< info "How to get the optimal capital stock equation for unconstrained banks"
...
$max_{C,K}r_{L}(K)K-r_{c}C-r_{D}(K-C,C)(K-C)$ s.t. $0<= C <= K$.
...
$r_{L}(L)= \alpha_{0}\alpha_{t}P_{ct}^{\theta+1}\eta_{c}\epsilon_{ct}-\alpha_{1}L=\alpha_{ct}P_{ct}^{\theta+1}-\alpha_{1}L$ where $\alpha_{ct}$ assimilates the county and time effects.
A way to capture costs of deposit is $r_{D}(D,C)=(\rho_{D}-\xi_{\omega})$, where $\omega=C/(C+D)$ equity-asset-ratio and $\rho_{D}$ and $\xi$ are constant.
The deposit cost for a bank is $\rho_{D}-\xi$, if the bank hast just capital. But if the deposit raises, they become more expensive and thus $\omega$ decreases. Total costs of capital can be represented via
$(\rho_{D}-\xi_{\omega})(1-\omega)K+r_{C}\omega K$.
Only the capital structure can affect the cost of capital, so minimizing the costs gives $\omega^*=(\rho_{D}-r_{C}+\xi)/(2\xi)$. Given these optimal costs of capital, the bank chooses the size to correlate the cost of raising capital with the return from lending it. 
...
$Y^{*}_{ct}$ displays the log production per person, our outcome of interest. $P_{ct}$ stands for the county population and certain effects are included via $\gamma_{t}$ (time effect), $\gamma_{e}$ (endogenous effect) and $\gamma_{c}$ (county effect).
...
```{r "2_1___Estimation_a"}
#< task
#Dat2 = ??.table("Data2.tab", header = TRUE, sep = "\t")%>%
...
### Effect of unconstrained entry

To show the effect of unconstrained entry we will only use counties with a bank, since the national banks in these counties aren't constrained by the minimum capital stock size. Also, we will just include time effects. Before starting with our first equation we must adapt our data set to these conditions.

**Task:**

Filter the data set in the shown way. Just delete `#` and press `check`.

```{r "2_1___Estimation_a_2"}
#< task
...
Let's describe this on a simple example. We have a sample of 50 people who we ask for their weight, which we note under `statement`. Afterwards we weight them and note the values under `scales`. All values are saved in the data set `weight`. Now we want to find out the relationship between `statement`and `scales`. To do this, we draw following simple linear regression model: $\hat {scales} = \alpha + \beta * scales$. Using R we can estimate this regression by `lm()`. (cf. Hatzinger, Hornik, Nagel (2011), pp.288)
...
#< info "mixed linear model"
In the info box above, you learned how linear models are compiled and how to solve them with R. Now, we want to use this information to calculate mixed linear models.

Mixed linear models have more than one explanatory variable. The equation we want to estimate looks generally like 
...
Regress log-production per person `ln_tval_pc` on `capitalstock50pc` and `factor(year)` of the data set `d1`. Make use of the function `lm()` and save it under the variable `lm1`.

```{r "2_1___Estimation_a_3"}
#< task
#lm1 = (??)
#>
lm1 = lm(ln_tval_pc~ capitalstock50pc + factor(year), data =d1)
```
...
*Std.Error:* Standard error measure the average amount that the estimates differ from the actual average value of the independent variable.
...
question: How much more output per person will be reached by adding a minimum-size bank to a county? Remember that "capitalstock50pc" is in units of adding a bank of minimum size to the county population. Type in a decimal with three degrees after the point, e.g.. 1.123.

answer: 6.115
#>

Perfect! If we add a minimum-size national bank to a county, we will get a higher output of `6%`.

#< info "t-Test for the regression coefficient"
We will introduce the *t-Test* for the regression coefficient $\beta$ to get a better knowledge about the summary-output. The important line for our analysis is the second of the `Coeffcients` part, here `capitalstock50pc`.
...
$t = b / se_{b}$ follows a t-distribution under the Zero-Hypothesis. $b$ is the regression coefficient and can be found under `Estimation`, here $b=0.061152$. $se_{b}$ is the standard error of $b$, here $se_{b}=0.009182$. Thus $t=0.061152/0.009182=6.660$, the same we find under `t value`in our output. Perfect! 

Now check `Pr(>|t|)`. It gives us the p-value for our test. Since $Pr(>|t|) = 3.42e-11 > 0$ we can reject the Zero-Hypothesis. Thus, we have a linear relationship between our variables of the regression (ct. Hatzinger, Hornik, Nagel (2011), pp.285).

#>

At the beginning of the estimation I claimed to include time effects in the regression. Using the `lm()` function I did this by introducing `factor(year)` as one of our explanatory variables. But there is a much nicer way to include effects in the estimation in R.


#< info "linear model with effects"
The package `lfe` has an amount of functions to estimate linear models with fixed effects. It is similar to `lm` but much faster. We will use the `felm` function of this package. The goal here is that we can separate the effects from the normal equation. Therefore, we can use it for huge data sets. It's defined as: `felm(y ~ x1 + x2 | f1 + f2 , data = dataname)`. `y ~ x1+x2` is the regression input, `f1+f2` are the multiple fixed effects.
#>
...
The estimated value for `capitalstock50pc` is `0.061152`, that means our capital per capita is `6%`. Adding a bank to a county will lead to a higher output per person of 6%. Remember that we just took care of counties with a bank and also didn't related to the minimum capital requirement, in this estimation. 



### Effect of constrained entry 

Before starting to regress the effect of unconstrained entry I will introduce the *OLS Estimation*, which I already mentioned before.
...
**Five Assumption for an optimal OLS:**
...
Knowing this, we will continue with our regression for calculating the effect of constrained entry.
We will include the minimal capital stock requirement of 50.000$ and calculate the effect of entry. Therefore, we compare counties that don't have a bank to counties with a bank, which opened at the minimum size. 
...
summary(felm(ln_tval_pc ~capitalstock50pc | year, data= d2))
```

Well done! I already showed how to interpret the `Coefficients` Part of a `summary()` output. For the sake of completeness, I will explain the whole output in the following info box.
...
Now, we want to check if our estimation fulfills the assumptions for an optimal OLS estimator.


#### 1. Assumption: 

*The linear model can be calculated as a sum of independent variables and a error term.*

We regressed `ln_tval_pc` on `capitalstock50pc` using time effects `year`. How can we write this in a equation?
Our dependent variable $Y$ is `ln_tval_pc`, our explanatory variable is `capitalstock50pc`. The time effects can be represented as $\beta_{t}$. In that case, we get the following regression model:

$$lntvalpc = \beta_{t} + \beta* capitalstock50pc + \epsilon $$
Thus, the first assumption for receiving th best OLS is valuable.




*Before checking the second assumption, we will review the third one.*


#### 3. Assumption:

*The disturbances have the uniform variance and aren't correlated with each other.*

To show that all residuals have the same variance, we have to show that **homoskedasticity** is given. In that case our OLS- estimator will be unbiased and the disturbances will have the same variance. The opposite of homoskedasticity is called **heterooskedasticity**. In that case the OLS estimator would be unbiased and consistent but no longer efficient (Kennedy (2008), Chapter 8).

#< info "Homoskedasticity & BP-Test"

*Homoskedasticity*

Wooldbridge (2016, p.82) **homoskedasticity** as following:

"The error u has the same variance given any value of the explanatory variables. In other words, $Var(u|X_{1},...,X_{k})=\sigma^2$".

We can test residuals if they are homoscedasticity or heteroscedasticity by using the *Breusch-Pagan -test*.

*BP-Test*

With the Breusch-Pagan-test you can test whether a linear regression model is homoskedasticity, meaning all residuals have the same variance. Therefore, it tests if the variance of the error term is dependent of the independent variables. If the test statistic has a p-value below an acceptable threshold e.g. p<0.5, then the null hypothesis of homoskedasticity is rejected and heteroskedasticity assumed. If you want to know how exactly a BP-Test is performed check Auer and Rottmann's book (2010, p.523 - 525) or Wooldridge(2016, Chapter 8.3).

In R we can use the function `bptest()` of the package `lmtest`.

```{r eval=FALSE}
library(lmtest)
bptest(var1 ~ var2, data=dat)
```
#>

**Task**

Load the package `lmtest` and perform a Breusch-Pagan Test for the variable `ln_tval_pc`and `capitalstock50pc` of the data set `d1`.
```{r}
#< task
#??
#>
library(lmtest)
bptest(ln_tval_pc ~ capitalstock50pc, data=d2)
```
#< award "Homoskedasticity"
Congratulations! You tested the regression model for homoskedasticity.
#>

The output gives us information about the kind of test which was performed, here `studentized Breusch-Pagan test`, the data which was used and the statistics of the test. `BP` shows the value of the test statistic, `df` the degrees of freedom an `p-value` displays the p-value. 

#< quiz "BP-test"
question: At the significance level of `5%`, would you reject the null hypothesis?
sc:
- yes
- no*
success: Correct!
...
Correlation shows if two variables have a linear relationship. The correlation coefficient between $Y$ and $X$ is defined as:
...
We will now calculate the correlation coefficient between our dependent and explanatory variable. Press `check`. 
...
Hence, our regression models fulfil the third assumption, the disturbances are uncorrelated and have the same variance. 

#### 2. Assumption: 

*The expected value of the error term is 0.*

$\mathbb{E}(\epsilon)=0$ should hold, meaning the OLS estimator $\hat{\beta}$ is unbiased. 
We already showed that we have a homoskedasticity regression, thus our OLS estimator is unbiased. We don't have endogenity, we have exogenity.

#< info "Endogenity and Exogenity"
*Endogenity* means, that the is relationship between the explanatory variable and the error term. The explanatory variable collerates with the residuals. Normally, all observable factory will be part of the error term of the regression. Thus, a variable which correlates with the explanatory variable will be as well included in the error term. Thus the explanatory variable correlates with the error term.
...
*In repeated samples, the observations of the independent variable can be consider as fixed.*

If you change the disturbance term of all endogenous variable will change simultaneously. Thus, the endogenous variable can't be considered as fixed in repeated sample(c.f. Kennedy (2008), p.171). 
Since we have exogenity, the fourth assumption holds for our regression.


#### 5. Assumption:

*The number of observations is greater than the amount of independent variables.*

Like Kennedy ((2008), p.192) declares in his book, it is nearly impossible to estimate `n` parameters with less than `n` numbers. Thus, the sample size is larger than the number of parameter they are estimating. Therefore assumption five also holds.


Our regression model estimates the **best linear unbiased estimator**.


## Indicator for having a county with a bank
...
#summary(??(?? ~ ?? | ??, data=d3))
#>
d3= filter(Dat2, insample, capitalstock<=50)
summary(felm(ln_tval_pc ~banks50c | year, data=d3))
```
#< award "Regression3"
...
```{r "2_1___Estimation_a_7",results='asis'}
#< task_notest
library(stargazer)
...
In the first regression we calculated, that the production per person is around `6%` if a bank was added.
...
In these regressions we just included time effects. 

## Exercise 3.1 - Clustering Standard Errors
<!-- evtl das kapitel weg  -->
...
# length(??(Dat2$??))
#>
length(unique(Dat2$countynum))
```

**Question**

#< quiz "Cluster"
question: How many clusters of "countynum"" do we have?
answer: 333
#>
...
$$
where $c = 1, \ldots, C$ indicates the cluster $c$. The sub matrices $y_c$, $X_c$ and $\varepsilon_c$ store values for cluster $c$ only. Hence, we get an individual regression equation for every cluster. In this case, the variance of $\hat{\beta}$ is given by
...
#< info "Clustering with felm()"
In Exercise 2.1 we described the structure of a `felm` function as following:
```{r optional=TRUE, eval=FALSE}
felm(y ~ x1 + x2 | f1 + f2 , data = dataname)
#'y ~ x1+x2' is the regression input
#'f1+f2' are the multiple fixed effects
```
But it is as well possible to insert clustered standard errors.
```{r optional=TRUE, eval=FALSE}
felm(y ~ x1 + x2 | f1 + f2  | 0 | cluster_variable , data = dataname)
```
The `0` in this formula is a placeholder for the option of covariates.
#>
...
#>
summary(felm(ln_tval_pc ~capitalstock50pc | year | 0 | countynum, data=d2))
```
#< award "Regression Expert"
Awesome! You learned how to correct for clustering, using `felm()`.
#>



## Exercise 3.2 - Estimation with Fixed effects

Before starting with this exercise, we have to load the data we used in Exercise 3 first. Press `edit` and `check`.
...
```
#< award "Regression4"
Great! You created your frist regression by using the plm-function.
#>
...
#< quiz "plm"
question: How much higher is the production per person output using fixed effect? Type in a decimel with one degree after the point.
answer: 17.8
#>
...
Perfect! We have already seen that gaining a national bank helps a county to get a higher production per person. I will summarise our results in the following table.
...
question: How many counties didn't had a national bank in our sample over different decades? Please type in an integer.
answer: 271
#>
...
question: How much would opening a national bank on minimum size increase the manufacturing production per capita in Mississippi? Type in your answer. Use `.` as separator for decimals and use three digits after the point.
answer: 6.203
#>
...
#< quiz "Table1"
question: How high is the increasing of farm production per person when adding banking with minimum capital stock? Please type in a decimal with three digits after the point. 
answer: 13.160
#>
...
Awesome! You are now a master of regressions. You know how to compile them, how to arrange the output and how to interpret the results.
#>
...
- Faraway, J.J. (2006): *Extending the Linear Model with R: Generalized Linear, Mixed Effects and Nonparametric Regression Models*. Boca Raton: Taylor and Francis Group, LLC.
